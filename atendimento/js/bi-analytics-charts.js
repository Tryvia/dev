...
if (typeof BIAnalytics !== 'undefined') {
    Object.assign(BIAnalytics.prototype, {
        renderCharts(metrics) {
            ...
            this.renderAvgResolutionTimeChart(metrics);  // Tempo Médio de Resolução por Pessoa/Time (exclui Melhorias/Projetos)
        ...
        renderTicketsPorTratativaChart(metrics, hoverIndex = null) {
            ...
            sorted.forEach(([name, count], i) => {
                ...
                // Região clicável (com função contains para detecção de hover)
                const regionX = labelWidth;
                const regionY = y;
                const regionW = barWidth + valueWidth;
                const regionH = barHeight;
                regions.push({
                    contains: (mx, my) => mx >= regionX && mx <= regionX + regionW && my >= regionY && my <= regionY + regionH,
        ...
        renderSLAChart(metrics, hoverIndex = null) {
            ...
            this.filteredData.forEach(ticket => {
                const slaStatus = this.isWithinSLA(ticket, 4);
        ...
        renderSLAByEntityChart(metrics, hoverIndex = null) {
            ...
            this.filteredData.forEach(ticket => {
                const rt = this.getResponseTimeMs(ticket);
                if (rt === null) return;
            ...
            this.setupCanvasTooltip(canvas, regions, (d) => {
                ...
                // Clique para paginação ou drill-down
                if (hit && hit.data) {
                    if (hit.data.action) {
                        ...
                        this.showSLAResponseTickets(hit.data.label);
                    }
                }
            });
        },
        ...
        renderSLAResolutionByEntityChart(metrics, hoverIndex = null) {
            ...
            this.setupCanvasTooltip(canvas, regions, (d) => {
                ...
                <div>Tempo médio: ${d.avgH}h</div>
                <div>✓ ${d.within} no prazo | ✗ ${d.outside} fora</div>
                <div style="font-size:0.7rem;color:#a1a1aa;margin-top:2px;">Clique para ver tickets</div>
            </div>`;
            }, (hit) => {
                const idx = hit ? hit.data.index : null;
                ...
                if (hit && hit.data) {
                    if (hit.data.action) {
                        ...
                        this.showSLAResolutionTickets(hit.data.label);
                    }
                }
            });
        },
        ...
        renderFirstResponseChart(metrics, hoverIndex = null) {
            ...
            const responseHours = [];
            this.filteredData.forEach(t => {
                const responseMs = this.getResponseTimeMs(t);
        ...
        renderRankingSLAChart(metrics, hoverIndex = null) {
            ...
            this.filteredData.forEach(t => {
                const p = t.custom_fields?.[treatField] || t[treatField];
                if (!p) return;
                const time = this.getResponseTimeMs(t);
                if (time === null) return;
        ...
        renderPendentesChart(metrics, hoverIndex = null) {
            ...
            const legendSpacing = Math.min(38, (height - 80) / items.length); // Espaçamento dinâmico
            let legendY = 45;
            items.forEach(([label, data], index) => {
                ...
                // Texto - uma linha só
                ctx.fillStyle = isHover ? '#ffffff' : this.colors.text;
                ctx.font = isHover ? 'bold 9px system-ui' : '9px system-ui';
                ctx.textAlign = 'left';
                const shortLabel = label.length > 12 ? label.slice(0, 10) + '..' : label;
                ctx.fillText(`${shortLabel}`, legendX + 12, legendY);
                ctx.fillStyle = this.colors.textMuted;
                ctx.font = '8px system-ui';
                ctx.fillText(`${data.count} (${percent}%)`, legendX + 12, legendY + 10);
        ...
        // ========== TEMPO MÉDIO DE RESOLUÇÃO POR ENTIDADE ==========
        // Exclui Melhorias e Projetos - apenas tickets de atendimento real
        // Suporta modo tratativa (cf_tratativa) e modo tags (cf_grupo_tratativa)
        renderAvgResolutionTimeChart(metrics, hoverIndex = null) {
            const canvas = document.getElementById('chartAvgResolutionTime');
            if (!canvas) return;

            // Usar campo baseado no modo atual (tratativa ou tags)
            const entityField = this.currentView === 'pessoa' ? 'cf_tratativa' : 'cf_grupo_tratativa';

            // Agregar por entidade - apenas tickets resolvidos, excluindo melhorias/projetos
            const byEntity = new Map();
            let totalExcluded = 0;

            this.filteredData.forEach(ticket => {
                // IMPORTANTE: Ignorar tipos de Melhoria e Projeto
                const typeNorm = (ticket.type || '').toString().toLowerCase().replace(/\s+/g, ' ').trim();
                ...
                // Ignorar outliers (> 720 horas = 30 dias)
                if (resolutionTimeHours <= 0 || resolutionTimeHours > 720) return;

                // Pegar entidades do campo apropriado
                const entities = ticket[entityField] ? ticket[entityField].split(/[,;\/]/).map(e => e.trim()).filter(e => e && this.selectedEntities.has(e)) : [];
            ...
            // Calcular métricas por entidade
            const allItems = Array.from(byEntity.entries()).map(([label, d]) => {
                const avgHours = d.times.length > 0 ? d.times.reduce((a, b) => a + b, 0) / d.times.length : 0;
                const minHours = d.times.length > 0 ? Math.min(...d.times) : 0;
                const maxHours = d.times.length > 0 ? Math.max(...d.times) : 0;
                const medianHours = d.times.length > 0 ? d.times.sort((a, b) => a - b)[Math.floor(d.times.length / 2)] : 0;
                return { label, avgHours, minHours, maxHours, medianHours, count: d.times.length, tickets: d.tickets };
            }).sort((a, b) => a.avgHours - b.avgHours); // Ordenar por menor tempo (melhor performance)
            ...
            if (allItems.length === 0) {
                ...
                ctx.font = '14px system-ui';
                ctx.textAlign = 'center';
                ctx.fillText('Sem dados de resolução', width / 2, height / 2 - 10);
                ctx.font = '11px system-ui';
                ctx.fillText('(excluindo Melhorias e Projetos)', width / 2, height / 2 + 10);
                if (totalExcluded > 0) {
                    ctx.fillText(`${totalExcluded} tickets de Melhoria/Projeto ignorados`, width / 2, height / 2 + 30);
                }
                return;
            }

            // Paginação
            if (!this.pagination.avgResolutionTime) this.pagination.avgResolutionTime = { page: 0, perPage: 10 };
            const perPage = this.pagination.avgResolutionTime.perPage;
            const totalPages = Math.ceil(allItems.length / perPage);
            const currentPage = Math.min(this.pagination.avgResolutionTime.page, totalPages - 1);
            this.pagination.avgResolutionTime.page = currentPage;
            const startIdx = currentPage * perPage;
            const items = allItems.slice(startIdx, startIdx + perPage);

            const padding = { top: 35, right: 80, bottom: 50, left: 120 };
            const chartWidth = width - padding.left - padding.right;
            const chartHeight = height - padding.top - padding.bottom - 20;
            const barHeight = 22;
            const gap = 4;
            ...
            const maxAvg = Math.max(...items.map(i => i.avgHours), 24); // Mínimo 24h para escala

            // Header informativo
            ctx.fillStyle = this.colors.textMuted;
            ctx.font = '10px system-ui';
            ctx.textAlign = 'left';
            ctx.fillText(`⚠️ Exclui Melhorias/Projetos (${totalExcluded} ignorados)`, 10, 14);

            // Meta de 24h (linha vertical)
            ...
            if (meta24hX <= width - padding.right) {
                ...
                ctx.lineWidth = 1;
                ctx.beginPath();
                ctx.moveTo(meta24hX, padding.top);
                ctx.lineTo(meta24hX, padding.top + chartHeight);
                ctx.stroke();
                ctx.restore();
                ...
                ctx.font = '9px system-ui';
                ctx.textAlign = 'center';
                ctx.fillText('Meta 24h', meta24hX, padding.top - 5);
            }
            ...
            items.forEach((item, index) => {
                ...
                // Cor baseada na performance (verde = rápido, vermelho = lento)
                const color = item.avgHours <= 24 ? this.colors.secondary : item.avgHours <= 48 ? this.colors.accent : this.colors.danger;
                const isHover = index === hoverIndex;
                ...
                ctx.fillStyle = color;
                const yy = isHover ? y - 3 : y;
                const hh = isHover ? barHeight + 6 : barHeight;
                ctx.fillRect(padding.left, yy, Math.max(w, 4), hh);
                if (isHover) {
                    ctx.strokeStyle = '#ffffffaa';
                    ctx.lineWidth = 1.2;
                    ctx.strokeRect(padding.left, yy, Math.max(w, 4), hh);
                }
                ctx.restore();
                ...
                // Valor (tempo médio formatado)
                ctx.textAlign = 'left';
                const displayTime = item.avgHours < 1 ? `${Math.round(item.avgHours * 60)}min` : item.avgHours < 24 ? `${item.avgHours.toFixed(1)}h` : `${(item.avgHours / 24).toFixed(1)}d`;
                ctx.fillText(displayTime, padding.left + Math.max(w, 4) + 6, y + barHeight / 2 + 4);

                // Quantidade de tickets
                ctx.fillStyle = this.colors.textMuted;
                ctx.font = '9px system-ui';
                ctx.fillText(`(${item.count})`, padding.left + Math.max(w, 4) + 45, y + barHeight / 2 + 4);

                regions.push({
                    contains: (mx, my) => mx >= padding.left && mx <= padding.left + Math.max(w, 4) && my >= y && my <= y + barHeight,
                    data: { ...item, index, globalIndex }
                });
            });
            ...
            if (totalPages > 1) {
                ...
                regions.push({
                    contains: (mx, my) => mx >= 10 && mx <= 40 && my >= btnY - 12 && my <= btnY + 5,
                    data: { isPagination: true, action: 'prev' }
                });
                regions.push({
                    contains: (mx, my) => mx >= width - 40 && mx <= width - 10 && my >= btnY - 12 && my <= btnY + 5,
                    data: { isPagination: true, action: 'next' }
                });
            }
            ...
            ctx.font = '9px system-ui';
            ctx.textAlign = 'center';
            ctx.fillText('🟢 ≤24h   🟡 24-48h   🔴 >48h', width / 2, height - 4);

            // Tooltips + hover + clique
            const self = this;
            const currentTotalPages = totalPages; // Capturar para o closure
            
            this.setupCanvasTooltip(canvas, regions, (d) => {
                if (d.isPagination) return null;
                ...
                const minDisplay = d.minHours < 1 ? `${Math.round(d.minHours * 60)} min` : d.minHours < 24 ? `${d.minHours.toFixed(1)}h` : `${(d.minHours / 24).toFixed(1)}d`;
                const maxDisplay = d.maxHours < 1 ? `${Math.round(d.maxHours * 60)} min` : d.maxHours < 24 ? `${d.maxHours.toFixed(1)}h` : `${(d.maxHours / 24).toFixed(1)}d`;
                return `
                <div style="display:flex;flex-direction:column;gap:4px;">
                    <strong>${d.label}</strong>
                    <div>⏱️ Tempo Médio: <b>${avgDisplay}</b></div>
                    <div style="font-size:0.85em;color:#a1a1aa;">
                        Min: ${minDisplay} | Max: ${maxDisplay}
                    </div>
                    <div>${d.count} tickets resolvidos</div>
                    <div style="font-size:0.75rem;color:#a1a1aa;margin-top:4px;">Clique para ver tickets</div>
                </div>
            `;
            }, (hit) => {
                // Hover callback - apenas para destacar barras (não paginação)
                if (!hit) {
                    if (self._lastMetrics) self.renderAvgResolutionTimeChart(self._lastMetrics, null);
                    return;
                }
                if (hit.data.isPagination) return; // Não fazer nada no hover de botões de paginação
                const idx = hit.data.index;
                if (self._lastMetrics) self.renderAvgResolutionTimeChart(self._lastMetrics, idx);
            }, (hit) => {
                // Click callback - paginação e modal de tickets
                if (!hit || !hit.data) return;
                
                if (hit.data.isPagination) {
                    // Clique nos botões de paginação
                    if (hit.data.action === 'prev' && self.pagination.avgResolutionTime.page > 0) {
                        self.pagination.avgResolutionTime.page--;
                        if (self._lastMetrics) self.renderAvgResolutionTimeChart(self._lastMetrics, null);
                    } else if (hit.data.action === 'next' && self.pagination.avgResolutionTime.page < currentTotalPages - 1) {
                        self.pagination.avgResolutionTime.page++;
                        if (self._lastMetrics) self.renderAvgResolutionTimeChart(self._lastMetrics, null);
                    }
                } else if (hit.data.tickets) {
                    // Clique nas barras - mostrar modal de tickets
                    self._showTicketsModal(hit.data.tickets, hit.data.label, 'Tempo Médio de Resolução');
                }
            });
        }
    });
